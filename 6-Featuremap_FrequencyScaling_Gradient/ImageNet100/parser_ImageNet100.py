...
def get_parser():
    ...
    # 🔵 === Seeds ===
    parser.add_argument('--seed1', type=int, default=1, help='global seed 1') #🎀 Best IN-100: 81.40% 'seed1: 1'  
    parser.add_argument('--seed2', type=int, default=2, help='global seed 2') #🎀 Best IN-100: 81.40% 'seed2: 2'
    ...
    parser.add_argument('--batch_size', type=int,  default=128, help='Batch size (default: 128)')  # 🎀 default:128
    ...
    parser.add_argument('--aa', type=str, default='rand-m5-mstd0.5-inc1', metavar='NAME', help='AutoAugment policy (default: rand-m5-mstd0.5-inc1)') #🎀 Best IN-100: 81.40% 'rand-m5-mstd0.5-inc1'  
    ...
    parser.add_argument('--model_name', default="LiteFA_Net", type=str,
        help="""Lightweight models (
                LiteFA_Net,
                TinyViT, VGG, 
                ConvNeXtV2-Atto, ConvNeXtV2-Femto, ConvNeXtV2-Nano, ConvNeXtV2-Tiny, ConvNeXtV2-Base,
                cct_7_3x1,
    ...
    parser.add_argument(
        '--mode_name',
        default="Ablation_cumulation",        # 🎀⭐ default: Full_LiteFA_Net | cum Lite-Net mode: Ablation_cumulation
        type=str,
        choices=[
            # ────────────────────────────────────────────────────────────────────────
            # 🧪🧪 === INDIVIDUAL ABLATION  ===
    ...
    parser.add_argument(
        '--cum_active',
        type=str,
        default="DWCONV,ECA,FNEB",      # 🎀 default: DWCONV,ECA,FNEB,FREQSPATIAL_MIXER,FREQGATECONV2D,FARC,FREQATTNFUSE | cum Lite-Net mode: DWCONV,ECA,FNEB
        help=(
    ...
    parser.add_argument('--act_name', default="gelu", type=str,
        help="Activation function (relu, gelu, tanh, sigmoid, swish, glu, tanhexp, fftgate, geglu)")    # 🎀 default:gelu | SOTA:XX
    ...
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    # ✅ === Feature Map Selection Mode ===
    # ───────────────────────────────────────────────────────────────────────────────────────────────── 
    parser.add_argument('--mode_featuremap', default="top", type=str,
                        help='Options: ["first","top","random"] | first: first N channels | top: highest activation magnitude | random: random N channels')
    
    parser.add_argument('--channels_featuremap', default=64, type=int, help='Number of channels to visualize (e.g., 64, 96, 128, 132, 144)')

    parser.add_argument('--use_txt_channels', type=bool, default=True,                     # 🎀⭐ default: False | cum Lite-Net mode: True
                        help='If True → read channel indices from existing txt file. '
                            'If False → compute using mode_featuremap (top/first/random).')
    
    parser.add_argument('--show_input_featuremap', type=bool, default=False,
                        help='If True → include input image in feature map visualization. '
                            'If False → show feature maps only.')

    parser.add_argument('--use_fsm_after_for_selection', type=bool, default=True,
        help='Toggle channel selection source: '
            'True = use FSM output (after), False = use pre-FSM features (before).')   

    parser.add_argument('--use_farc_after_for_selection', type=bool, default=True,
        help='Toggle channel selection source: '
            'True = use FARC output (after), False = use pre-FARC features (before).')          
    # ─────────────────────────────────────────────────────────────────────────────────────────────────


    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    # ✅ === Frequency Scaling Gradient Analysis ===
    # ───────────────────────────────────────────────────────────────────────────────────────────────── 
    parser.add_argument('--run_freq_analysis', type=bool, default=False,            # 🎀⭐ default: Feature map analysis mode: False | freq_scaling_gradient analysis mode: True
        help='Toggle frequency-gradient analysis: True = run analysis, False = skip.')
    # ─────────────────────────────────────────────────────────────────────────────────────────────────    
