...
#####------------------------- NOTE LiteFA_Net IMAGENET100 NOTE -----------------------------------------------------#####
##########################################################################################################################
######################|--------------------------------------------------------------|####################################
...
# ────────────────────────────────────────────────────────────────────────────────────────────────
# ✅ Import from utils_LiteFA_Net folder
from models.utils_LiteFA_Net.ablation_mode import (
    USE_FREQGATECONV2D,
    USE_FARC,
...
class FreqGateConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):
        ...
        self.gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),                                  # ⭐ GAP is reduendant here because freq_amp is already collapsed (B,C,H,W) → (B, C, 1, 1) using ".mean(dim=(-2, -1), keepdim=True" 
    ...
    def forward(self, x, fft_x=None):
        # # ❗ KEEP this (input analysis if needed, but NOT Z)
        # self.fgconv_last_x_in = x             # ⭐🎀 Store input for later analysis
        # self.fgconv_last_x_in.retain_grad()   # ⭐🎀 Store input for later analysis
        ...
        freq_amp = torch.abs(fft_x).mean(dim=(-2, -1), keepdim=True)  # 🔵 a_c^(n)
        # ────────────────────────────────────────────────────────────────────────────────────────────────

        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # 🔑 ======= Channel gating from frequency amplitude =======
        # 🔑 Gate
        gate = self.gate(freq_amp)                                    # 🔵 G_fgConv^(n) 
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # 🧠 ======= Main convolution =======
        out = self.conv(x)                                            # 🔵 Z_fgConv^(n)
        # ────────────────────────────────────────────────────────────────────────────────────────────────        
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        pure_gated = out * gate                                       # 🔵 Z ⊙ G
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # 🔧 ======= Apply gate with residual blending for stability =======
        # 🔵 ======= FINAL OUTPUT =======  
        # gated_out = out * gate + self.alpha * out                  # 🔵  Y_fgConv^(n) = Z ⊙ G + αZ
        gated_out = pure_gated + self.alpha * out                    # 🔵  Y_fgConv^(n) = Z ⊙ G + αZ
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # ⭐ 🎀 ======= ⚠️ STORE FOR ANALYSIS (NO detach)  =======        
        self.fgconv_last_freq_amp = freq_amp                   # ⭐ 🎀 a_c^(n)
        self.fgconv_last_gate = gate                           # ⭐ 🎀 G_fgConv^(n) 
        self.fgconv_last_z = out                               # ⭐ 🎀 Z_fgConv^(n): pre-gating convolutional feature response
        self.fgconv_last_pure = pure_gated                     # ⭐ 🎀 Z ⊙ G
        self.fgconv_last_y = gated_out                         # ⭐ 🎀 Y_fgConv^(n) = Z ⊙ G + αZ
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # ⭐ 🎀 enable gradient access
        if torch.is_grad_enabled():        
            self.fgconv_last_freq_amp.retain_grad()                # ⭐ 🎀 ∂L/∂a_c^(n)
            self.fgconv_last_gate.retain_grad()                    # ⭐ 🎀 ∂L/∂G_fgConv^(n)   
            self.fgconv_last_z.retain_grad()                       # ⭐ 🎀 ∂L/∂Z_fgConv^(n)
            self.fgconv_last_pure.retain_grad()                    # ⭐ 🎀 ∂L/∂(Z ⊙ G)
            self.fgconv_last_y.retain_grad()                       # ⭐ 🎀 ∂L/∂Y_fgConv^(n)
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        return gated_out
...
class FreqSpatialMixer(nn.Module):
    ...
    def __init__(self, channels):
        ...
        self.freq_proj = nn.Sequential(nn.AdaptiveAvgPool2d(1),             # ⭐ GAP
                                       nn.Conv2d(channels, channels, 1),    # ⭐ 1x1 Conv
                                       nn.Sigmoid())
    ...
    def forward(self,x,fft_x=None):
        ...
        if fft_x is None or fft_x.shape[-2:]!=x.shape[-2:]:
            fft_x = safe_fft2_amp(x)
        fft_mag = torch.abs(fft_x)                           # 🔵 |F(X_c^(0))| 

        # ─────────────────────────────────────────────
        # 🧠 Frequency and Spatial Features
        # ─────────────────────────────────────────────        
        freq_feat = self.freq_proj(fft_mag)                 # 🔵 F_fsm^(0)
        spatial_feat = self.spatial_proj(x)                 # 🔵 S_fsm^(0)  
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        mix = freq_feat * spatial_feat                      # 🔵 M_fsm^(0)                        

        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # 🔑 Gate
        gate = self.gate(mix)                               # 🔵 G_fsm^(0) 

        # ────────────────────────────────────────────────────────────────
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        pure_gated = gate * mix                             # 🔵 G ⊙ M
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -        
        # ────────────────────────────────────────────────────────────────
        # 🔵 ======= FINAL OUTPUT =======   
        out =  x + pure_gated                               # 🔵  Y_fsm^(0) = X + G ⊙ M
        # ────────────────────────────────────────────────────────────────

        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # ⭐ 🎀 ======= ⚠️ STORE FOR ANALYSIS (NO detach)  =======
        self.fms_last_before = x                           # ⭐ 🎀 X_before FSM | (B,C,H,W)           
        self.fsm_last_fft_mag = fft_mag                    # ⭐ 🎀 |F(X_c^(0))|
        self.fsm_last_gate = gate                          # ⭐ 🎀 G_fsm^(0) 
        self.fsm_last_m = mix                              # ⭐ 🎀 M_fsm^(0)
        self.fsm_last_pure = pure_gated                    # ⭐ 🎀 G ⊙ M
        self.fsm_last_y = out                              # ⭐ 🎀 Y_fsm^(0) = X + G ⊙ M
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # ⭐ 🎀 enable gradient access
        if torch.is_grad_enabled():   
            self.fms_last_before.retain_grad()             # ⭐ 🎀 ∂L/∂X_before^(0)  
            self.fsm_last_fft_mag.retain_grad()            # ⭐ 🎀 ∂L/∂|F(X_c^(0))|
            self.fsm_last_gate.retain_grad()               # ⭐ 🎀 ∂L/∂G_fsm^(0)   
            self.fsm_last_m.retain_grad()                  # ⭐ 🎀 ∂L/∂M_fsm^(0)
            self.fsm_last_pure.retain_grad()               # ⭐ 🎀 ∂L/∂(G ⊙ M)
            self.fsm_last_y.retain_grad()                  # ⭐ 🎀 ∂L/∂Y_fsm^(0)
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # ────────────────────────────────────────────────────────────────────────────────────────────────
...
class FreqAttnFuse(nn.Module):
    ...
    def forward(self,x_early,x_late,fft_early=None,fft_late=None):
        ...
        f_early = torch.abs(fft_early).mean(dim=(-2,-1))                                 # ⭐ Collapse DONOT keep spatil dims (mean(dim=(-2,-1))  → removes H and W) | (B,C)
        f_late = torch.abs(fft_late).mean(dim=(-2,-1))

        # ────────────────────────────────────────────────────────────────────────────────────────────────
        attn = self.fc(torch.cat([f_early,f_late],dim=1)).unsqueeze(-1).unsqueeze(-1)    # ⭐ unsqueeze (B, C) → (B, C, 1, 1)  | ← reshape for broadcasting
        ...
        fused = x_late + pure_gated_early + self.beta * pure_gated_late        # ⭐ 🎀 x_late is reference signal refined                    
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -    
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -          
        # ────────────────────────────────────────────────────────────────
        ...
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # ⭐ 🎀 ======= STORE FOR ANALYSIS (FARC STYLE) =======
        self.faf_last_x_late = x_late            # ⭐ baseline
        self.faf_last_x_early = x_early          # ⭐ skip

        self.faf_last_freq_early = f_early       # ⭐ frequency early
        self.faf_last_freq_late = f_late         # ⭐ frequency late

        self.faf_last_attn = attn                # ⭐ forward attention
        self.faf_last_rev_attn = rev_attn        # ⭐ reverse attention

        self.faf_last_pure_early = pure_gated_early
        self.faf_last_pure_late  = pure_gated_late

        self.faf_last_after = fused              # ⭐ FAF output (KEY)

        # ────────────────────────────────────────────────────────────────
        # ⭐ enable gradient access
        if torch.is_grad_enabled():
            self.faf_last_x_late.retain_grad()
            self.faf_last_x_early.retain_grad()

            self.faf_last_attn.retain_grad()
            self.faf_last_rev_attn.retain_grad()

            self.faf_last_pure_early.retain_grad()
            self.faf_last_pure_late.retain_grad()

            self.faf_last_after.retain_grad()
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        return fused
...
class FARC(nn.Module):
    ...
    def forward(self, x, fft_x):
        ...
        amp = torch.abs(fft_x).mean(dim=(-2, -1), keepdim=True)   # ⭐ Collapse but keep spatil dims (mean(dim=(-2,-1))  → removes H and W) | (B,C,1,1)
        ...
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # ⭐ 🎀 ======= ⚠️ STORE FOR ANALYSIS (NO detach)  =======        
        self.farc_last_before = x                        # ⭐ 🎀 X                | (B,C,H,W)
        self.farc_last_amp   = amp                       # ⭐ 🎀 a_c^(n)          | (B,C,1,1)
        self.farc_last_scale = scale                     # ⭐ 🎀 G_farc^(n)       | (B,C,1,1)
        self.farc_last_after = out                       # ⭐ 🎀 FARC(X)=X ⊙ G   | (B,C,H,W)
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - -
        # ⭐ 🎀 enable gradient access
        if torch.is_grad_enabled():   
            self.farc_last_before.retain_grad()          # ⭐ 🎀 ∂L/∂X_c^(n)     
            self.farc_last_amp.retain_grad()             # ⭐ 🎀 ∂L/∂a_c^(n)
            self.farc_last_scale.retain_grad()           # ⭐ 🎀 ∂L/∂G_farc^(n)   
            self.farc_last_after.retain_grad()           # ⭐ 🎀 ∂L/∂(X ⊙ G) 
...
# ────────────────────────────────────────────────────────────────────────────────────────────────
        ...
        # # 🔴 SANITY (inside model)
        # print(
        #     f"[SANITY-IN] LiteFA_Net: num_classes arg = {num_classes} | "
        #     f"fc.out_features = {self.fc[-1].out_features}"
        # )
        # # ────────────────────────────────────────────────────────────────────────────────────────────────
...
modelspec_path = {"log_modelspec_history": f'./models/Results/model_{tag_path}_{args.dataset_name}_{args.act_name}_{args.main_opt_name}_{mode_tag}_Seed{args.seed1}_{args.seed2}_{args.customize_inputsize}x{args.customize_inputsize}.txt'}
...
log_modelspec_history.append(f"📊🏷️1. ============= {tag_path} Complexity Check ===========================")
log_modelspec_history.append(f"================================================================================================")


# 🧩 ======= Create LiteFA_Net-S from scratch for ImageNet-100 ======= 
model = LiteFA_Net()
model.eval()
...
log_modelspec_history.append(f"✅ MACs: {macs}")
log_modelspec_history.append(f"✅ Parameters: {params}")
log_modelspec_history.append(f"⚖️ model={args.model_name}-{args.LiteFA_Net_variant} | state_dim={args.state_dim} | layers={args.layers} "
      f"| fc_dropout={args.dropout} | down_sampling_i={model.down_i}"
      )
log_modelspec_history.append(f"🔬 Ablation: {get_ablation_signature()}")
...
log_modelspec_history.append(
    ...
    f"model.fc.out_features = {model.fc[-1].out_features}"
)
...
log_modelspec_history.append(f"\n==============================================================================================")
log_modelspec_history.append(f"📐🏷️3. ============= {tag_path} Architectural Specification  ===============")
log_modelspec_history.append(f"================================================================================================")
...
# -------------------------
# Initial Feature Extraction (n = 0)
# -------------------------
log_modelspec_history.append("🟦 Initial Feature Extraction (n = 0):")
log_modelspec_history.append(f"  ├─ init_map (1×1 Conv): {model.init_map}")
log_modelspec_history.append(f"  ├─ FSM: {model.freqspat_mixer}")
log_modelspec_history.append(f"  ├─ DWConv: {model.dwconv_block}")
log_modelspec_history.append(f"  └─ FARC: {model.rescalib}")

# -------------------------
# Feature Processing Blocks
# -------------------------
log_modelspec_history.append("\n🟩 Feature Processing Blocks (n = 1 … L):")
for i, block in enumerate(model.blocks):
    n = i + 1
    stride = block[0].conv.stride
    down = "↓ downsample" if stride[0] > 1 else "—"
    log_modelspec_history.append(f"\n  Block n = {n} | stride={stride} {down}")

    log_modelspec_history.append(f"    ├─ FGConv: {block[0]}")
    log_modelspec_history.append(f"    ├─ ECA: {block[1]}")
    log_modelspec_history.append(f"    ├─ BatchNorm: {block[2]}")
    log_modelspec_history.append(f"    └─ GELU: {block[3]}")

    if i == 0 and hasattr(model, "fneb"):
        log_modelspec_history.append("    ⭐ FNEB applied (early nonlinear expansion)")
    if i == 1:
        log_modelspec_history.append("    🔖 Early skip feature extracted")

# -------------------------
# Fusion
# -------------------------
log_modelspec_history.append("\n🟨 Feature Fusion:")
log_modelspec_history.append(f"  ├─ FAF: {model.fuse}")
log_modelspec_history.append(f"  └─ Post-fusion BN + GELU: {model.post_fuse}")

# -------------------------
# Classifier
# -------------------------
log_modelspec_history.append("\n🟥 Classifier:")
log_modelspec_history.append(f"  ├─ GAP: {model.pool}")
log_modelspec_history.append(f"  ├─ LayerNorm: {model.pre_fc_norm}")
log_modelspec_history.append(f"  ├─ SE: {model.pre_fc_se}")
log_modelspec_history.append(f"  └─ MLP: {model.fc}")
