...
MY_Model_PATH = r"C:\Users\emeka\Research\ModelCUDA\Neural_Network\ImageNet100"
...
# ────────────────────────────────────────────────────────────────────────────────────────────────
# 📜 ============  Imput parser safe for afno becauseof --fno-bias, etc  ========================
# ────────────────────────────────────────────────────────────────────────────────────────────────
...
# 🔴 === GFNet Variants ===
        ...
        "gfnet-xs", "gfnet-ti", "gfnet-s", "gfnet-b",
        "gfnet-h-ti", "gfnet-h-s", "gfnet-h-b"
    ]:
    try:
        from models.GFNet import (
            GFNet,
            GFNetPyramid,
        )
        print(f"✅ {args.model_name} imported successfully from GFNet.py!")
    except ModuleNotFoundError as e:
        print(f"❌ Import failed: {e}")
        print(f"🔍 Check that 'GFNet.py' exists inside: {MODELS_PATH}")

# ─────────────────────────────────────────────────────────────────────────────────────────────────
# 🔵 === AFNO ===
elif args.model_name == "afno":
    try:
        from models.afnonet import AFNONet
        print("✅ AFNONet imported successfully!")
    except ModuleNotFoundError as e:
        print(f"❌ Import failed: {e}")
        print(f"🔍 Check that 'afnonet.py' exists inside:{MODELS_PATH}")

# ─────────────────────────────────────────────────────────────────────────────────────────────────
# 🔵 === FFC ResNet-50 ===
elif args.model_name == "ffc_resnet50":
    try:
        from models.ffc_resnet import ffc_resnet50
        print("✅ ffc_resnet50 imported successfully!")
    except ModuleNotFoundError as e:
        print(f"❌ Import failed: {e}")
        print(f"🔍 Check that 'ffc_resnet.py' exists inside:{MODELS_PATH}")
# ─────────────────────────────────────────────────────────────────────────────────────────────────

# 🔵 === FFC ResNet-101 ===
elif args.model_name == "ffc_resnet101":
    try:
        from models.ffc_resnet import ffc_resnet101
        print("✅ ffc_resnet101 imported successfully!")
    except ModuleNotFoundError as e:
        print(f"❌ Import failed: {e}")
        print(f"🔍 Check that 'ffc_resnet.py' exists inside:{MODELS_PATH}")
...
# 🔵 === FFC ResNet-152 ===
    ...
    try:
        from models.ffc_resnet import ffc_resnet152
        print("✅ ffc_resnet152 imported successfully!")
    except ModuleNotFoundError as e:
        print(f"❌ Import failed: {e}")
        print(f"🔍 Check that 'ffc_resnet.py' exists inside:{MODELS_PATH}")
...
# 🔵 === FFC ResNet-200 ===
    ...
    try:
        from models.ffc_resnet import ffc_resnet200
        print("✅ ffc_resnet200 imported successfully!")
    except ModuleNotFoundError as e:
        print(f"❌ Import failed: {e}")
        print(f"🔍 Check that 'ffc_resnet.py' exists inside:{MODELS_PATH}")
# ─────────────────────────────────────────────────────────────────────────────────────────────────
...
def train(epoch, net, trainloader, device, train_criterion, optimizer, lr_scheduler, num_epochs, model_ema=None, mixup_fn=None): 
    ...
    with tqdm(enumerate(trainloader), total=len(trainloader), desc=f"Epoch {epoch}") as progress:
        for batch_idx, (inputs, targets) in progress:
            ...
            if args.prefetcher:
                ...
                # ✔️ APPLY MIXUP ONLY WHEN PREFETCHER IS OFF
            ...
            # # ❌ DO NOT convert soft-labels to hard labels before loss
            # #    Just keep `targets` as returned by mixup_fn.

            # # ✅ Use a criterion that matches the target type:
            # #    - If mixup is active: use SoftTargetCrossEntropy (expects soft labels)
            # #    - If mixup is off: use LabelSmoothingCrossEntropy or standard CE                
            # # ─────────────────────────────────────────────────────────────────────────────────────────────────
           ...
           # ─────────────────────────────────────────────────────────────────────────────────────────────────
            ...
            if args.use_amp:
                ...
                with torch.cuda.amp.autocast(): 
                    outputs = net(inputs)
                    loss = loss_fn(outputs, targets)         # ✅ soft targets for SoftTargetCE and hard targets for normal
                    # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - - -
            ...
            # 🔧 === when Mixup or CutMix (Soft labels) ===
            if targets.ndim == 2:  
                targets_class = targets.argmax(dim=1)
            else: # ✅ when no mixup (hard)
...
if __name__ == '__main__':
    ...
    elif args.model_name == 'gfnet-xs':
        # 🧩 ======= Create gfnet-xs from scratch for ImageNet-100 ======= 
        net = GFNet(
            img_size=args.customize_inputsize, 
            patch_size=16, 
    ...
    elif args.model_name == 'gfnet-ti':
        # 🧩 ======= Create gfnet-ti from scratch for ImageNet-100 =======
        net = GFNet(
            img_size=args.customize_inputsize, 
            patch_size=16, 
            in_chans=args.input_channels, 
    ...
    elif args.model_name == 'gfnet-s':
        # 🧩 ======= Create gfnet-s from scratch for ImageNet-100 =======
        net = GFNet(
            img_size=args.customize_inputsize, 
            patch_size=16, 
            in_chans=args.input_channels, 
    ...
    elif args.model_name == 'gfnet-b':
        # 🧩 ======= Create gfnet-b from scratch for ImageNet-100 =======
        net = GFNet(
            img_size=args.customize_inputsize, 
            patch_size=16, 
            in_chans=args.input_channels, 
            num_classes=args.num_classes,
            embed_dim=int(512 * 2),                    # 🔖 used from paper:512 (61.76) | test2: 512 * 1.5 (59.82) | test3: 512 * 2 ()
            depth=19, 
            mlp_ratio=4, 
            drop_path_rate=0.25,                         # 🔖 used from paper:0.25 (61.76) | test2: 0.0 (61.36)
            norm_layer=partial(nn.LayerNorm, eps=1e-6)       
        )
    ...
    elif args.model_name == 'gfnet-h-ti':
        net = GFNetPyramid(
            img_size=args.customize_inputsize, 
            patch_size=4,
            num_classes=args.num_classes, 
    ...
    elif args.model_name == 'gfnet-h-s':
        net = GFNetPyramid(
            img_size=args.customize_inputsize, 
            patch_size=4,
            num_classes=args.num_classes,
    ...
    elif args.model_name == 'gfnet-h-b':
        net = GFNetPyramid(
            img_size=args.customize_inputsize, 
            patch_size=4,
            num_classes=args.num_classes, 
    ...
    # ────────────────────────────────────────────────────────────────────────────────────────────────
        ...
        args.mixing_type = 'afno'     # 🧩 link model_name to mixer

        # 🧩 ======= Create afno from scratch for ImageNet-100 ======= 
        net = AFNONet(
            img_size=args.customize_inputsize,
            patch_size=16,
            in_chans=args.input_channels, 
            num_classes=args.num_classes,
            embed_dim=args.hidden_size,                  # 🔖 default in original class: 768 | used from paper:384
            depth=args.num_layers,                       # 🔖 default in original class: 12 | used from paper:12
        )
        print(f"✅ Initialized model with {net}.")
    # ────────────────────────────────────────────────────────────────────────────────────────────────
    ...
    # ────────────────────────────────────────────────────────────────────────────────────────────────
        ...
        raise ValueError(
            ...
            f"MobileNetV3-L, MobileNetV3-S, "
            f"ResNet-18,"
            f"gfnet-xs, gfnet-ti, gfnet-s, gfnet-b, gfnet-h-s, gfnet-h-b,"
            f"afno,"
            f"ffc_resnet50, ffc_resnet101, ffc_resnet152, ffc_resnet200, "
            f"]."
        )
    ...
    for epoch in range(start_epoch, num_epochs):   # ⚠️ Runs training for num_epochs
        ...
        train(epoch, net, trainloader, device, train_criterion, optimizer, lr_scheduler, num_epochs, model_ema, mixup_fn=mixup_fn) 
    ...
    ########################################################################################################################
    ####-------| NOTE 1️⃣2️⃣ MACs + REPORT LOGGING | XXX ------------------------------------------------####################
    ########################################################################################################################

    # ────────────────────────────────────────────────────────────────────────────────────────────────
    # ⚙️ === Compute MACs and FLOPs ===
    # ────────────────────────────────────────────────────────────────────────────────────────────────
    # 🔖=== Use timm-resolved input size ===
    in_ch, in_h, in_w = data_config['input_size']
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - - - 
    if args.model_name == "LiteFA_Net":
        # ❗=== LiteFA_Net does NOT need special prep/reset for ptflops ===
        macs, params = get_model_complexity_info(
            net, (in_ch, in_h, in_w), as_strings=True, print_per_layer_stat=False
        )
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - -  - - - - - - - - 
    else:
    ...
    if args.model_name == "LiteFA_Net":
        ...
        # 📌📌 ========  LiteFA_Net =====================================================================
        # ─────────────────────────────────────────────────────────────────────────────────────────────────   
        tag_report = f"{args.model_name}-{args.LiteFA_Net_variant} (from scratch, {args.dataset_name} @ {args.customize_inputsize}x{args.customize_inputsize})"

    else:
        # ─────────────────────────────────────────────────────────────────────────────────────────────────
        # 📌📌 ========  SOTA Models =====================================================================
        # ─────────────────────────────────────────────────────────────────────────────────────────────────
        tag_report = f"{args.model_name} (from scratch, {args.dataset_name} @ {args.customize_inputsize}x{args.customize_inputsize})"
